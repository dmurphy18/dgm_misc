#!/usr/bin/env sh
#
# All-in-one installer for Salt on AIX
# 
...
_log "Checking if initscript dir ${initscript_dir} exists"
test -d "$initscript_dir" || mkdir -p "$initscript_dir" || _error "Unable to create initscript directory"
...
for initscript in $initscripts; do
    ...
    _display "Installing $initscript initscript as ${initscript_dir}/${initscript}"
    cat <<@EOF >"${initscript_dir}/${initscript}"
#!/bin/sh
...
exit 0
@EOF
    chmod +x "${initscript_dir}/${initscript}"
done
...
_log "Checking if executable dir ${executable_dir} exists"
test -d "$executable_dir" || mkdir -p "$executable_dir" || _error "Unable to create executable directory"

for wrapper in $wrappers; do
    _display "Installing $wrapper executable wrapper as ${executable_dir}/${wrapper}"
    cat <<@EOF >"${executable_dir}/${wrapper}"
#!/bin/sh

export LD_LIBRARY_PATH=${install_dir}/lib:${install_dir}/lib64:${ld_path}
export PYTHONPATH=$pythonpath

"${install_dir}/bin/${wrapper}" "\$@"

exit \$?
@EOF
    chmod +x "${executable_dir}/${wrapper}"
done
